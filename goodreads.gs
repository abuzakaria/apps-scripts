function GoodReads() {
    ...
    if (book) {
        ...
        sheet.appendRow([book.title, book.author, book.pages, book.year, book.genre]);
    }
    ...
    if (response.getResponseCode() === 200) {
        ...
        if (id) {
            ...
            if (bookResponse.getResponseCode() === 200) {
                ...
                var shelves = book.getChild("popular_shelves").getChildren("shelf");
                var genre = "Fiction";
                for (var i =0; i< shelves.length; i++) {
                  var name = shelves[i].getAttribute("name").getValue();
                  if (name === "non-fiction" || name === "nonfiction") {
                    genre = "Non-fiction";
                    break;
                  }
                }
